...
# argument 2 => Mode of SCT (glmGamPoi/glmGamPoi2/offset-10/offset-100/offset-Inf/offset_shared_theta-100/offset_shared_theta-500/offset_shared_theta-1000)
...
ggsave(plot = p, filename = file.path(output_prefix, "umap.pdf"))
...
p <- residualVarPlot(gene_attr) #+ ggtitle(ncells)
plot_pdf <- gsub(".csv", "_residvar.png", output_geneattr)
ggsave(plot = p, filename = plot_pdf, width = 6, height = 2)

timediff <- as.numeric(times$stop_time - times$start_time, units = "secs")
timedf <- data.frame(method = sct_method, timediff = timediff)

output_scttime <- file.path(output_prefix, "sct_time.csv")
write.csv(
  timedf,
  output_scttime
)
